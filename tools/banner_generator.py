...
class BannerGenerator(object):
    ...
    def __init__(self, size, server, netinfo):
        ...
        self._fontLarge = ImageFont.truetype("fonts/FreeMono.ttf",16)
        self._fontLargeBold = ImageFont.truetype("fonts/FreeMonoBold.ttf",16)
        self._fontSmall = ImageFont.truetype("fonts/FreeMono.ttf",10)
        self._image = None
    ...
    def generate(self, ip, format="png"):
        ...
        # Title
        name = u''.join(self._netinfo.name if is_online else self._server.name)
        ...
        if is_online:
			...
			map = u''.join(self._netinfo.map if is_online else ". . .")
			...
			draw.text((15, 28), "%s: %s - %s: %s - %s: %s - %s: %s - %s: %s" 
								% (_('Players'), players,
                                   _('Map'), map, 
